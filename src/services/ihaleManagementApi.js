...
// Enes Doğanay | 6 Mayıs 2026: İhale ek dosyası için signed URL oluştur
export const createSignedIhaleFileUrl = async (fileRef) => {
    if (fileRef.url && fileRef.url.startsWith('http')) return fileRef.url;
    if (!fileRef.path) return null;
    const { data, error } = await supabase.storage.from('ihale-ekleri').createSignedUrl(fileRef.path, 300);
