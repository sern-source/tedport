                ...
                {isExpanded && (
                    <div className="tom-offer-card__detail">
                        ...
                        <div className="tom-offer-card__footer">
                            ...
                            {/* Enes Doğanay | 9 Mayıs 2026: Önceki teklifler butonu — sadece revize varsa göster */}
                            {(offer.revize_sayisi > 0) && (
                                <button className="tom-btn tom-btn--revisions" onClick={e => { e.stopPropagation(); onOpenRevisions(offer); }}>
                                    <span className="material-symbols-outlined">history</span>
                                    Önceki Teklifler
                                    <span className="tom-btn-revisions__count">{offer.revize_sayisi}</span>
                                </button>
                            )}
                            {/* Enes Doğanay | 9 Mayıs 2026: İletişime Geç kaldırıldı, Statüyü Değiştir kompakt yapıldı */}
                            {offerDurum === 'kabul' && (<div className="tom-offer-card__footer-right"><div className="tom-status-dropdown-wrap"><button className="tom-btn tom-btn--status-change" onClick={() => setStatusDropdownId(statusDropdownId === offer.id ? null : offer.id)}><span className="material-symbols-outlined">swap_horiz</span>Statüyü Değiştir</button>{statusDropdownId === offer.id && <div className="tom-status-dropdown"><button onClick={() => { onStatusConfirm(offer.id, 'gonderildi'); setStatusDropdownId(null); }}><span className="material-symbols-outlined" style={{ color: '#2563eb' }}>hourglass_top</span>Değerlendiriliyor</button><button onClick={() => { onReject(offer.id); setStatusDropdownId(null); }}><span className="material-symbols-outlined" style={{ color: '#dc2626' }}>cancel</span>Reddedildi</button></div>}</div></div>)}
